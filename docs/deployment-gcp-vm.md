...
## 10. Cách cập nhật Code sau này (Tự động)

Tôi đã tạo cho bạn 2 script trong thư mục `scripts/` để bạn có thể cập nhật App từ máy local chỉ với 2 lệnh:

### Bước 1: Push code lên Git
Chạy lệnh này trên máy local để commit và push toàn bộ thay đổi mới:
```bash
./scripts/deploy-git.sh "Nội dung commit của bạn"
```

### Bước 2: Cập nhật VM
Chạy lệnh này để VM tự động pull code mới, install library và restart app:
```bash
...
## 11. Đồng bộ Database (SQLite)

Vì SQLite là một file đơn lẻ (`data/app.db`), bạn có thể dễ dàng đồng bộ dữ liệu giữa máy local và server bằng 2 script sau:

### Đẩy dữ liệu từ Local lên VM (Ghi đè VM)
Nếu bạn đã cấu hình xong xuôi ở máy local và muốn đẩy dữ liệu đó lên server:
```bash
./scripts/sync-db-push.sh
```

### Tải dữ liệu từ VM về Local (Ghi đè Local)
Nếu bạn muốn lấy dữ liệu thực tế từ khách hàng/chatbot trên server về máy local để phân tích hoặc debug:
```bash
./scripts/sync-db-pull.sh
```
*(Script này sẽ tự động tạo một bản backup `.bak` cho file DB local của bạn trước khi ghi đè để đảm bảo an toàn).*
